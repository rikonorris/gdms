...
function size() {
    ...
    $('#content').height($(window).height() - 98);
}
...
function onload() {
    ...
    $('#space_quota_string').html(get_human_string(space_consumed) + ' з ' + get_human_string(max_space) + ' використано');
}
...
function show_files(id) {
    ...
    for (d in dirs) {
        $('#content').append("<div class='file' title='" + dirs[d]['name'] + "'" +
            "onmousedown='rightclick_handle(event,\"" + dirs[d]['id'] + "\",\"dir\");' onclick=\"show_files(" + dirs[d]['id'] + ")\"><div class='thumbnail'>" +
    ...
    for (f in files) {
        ...
        $('#content').append(
            ...
            "onclick='download(\"" + get_path(id).substr(1) + files[f] + "\");'" +
            "><div class='thumbnail'>" +
...
function show_directories(ds) {
    ...
    for (d in ds) {
        ...
        html += "<div class='directory " + (id == dir_id ? 'current_directory' : '') + "' id='" + id + "'><div class='directory-sign' onclick='change_sign(" + id + ")'>" + sign + "</div>" +
            ...
            "<div class='directory-name' >" + (d == '' ? 'Кореневий каталог' : d) + "</div></div></div>\n";
        if (ds[d]['open'] == 'yes')
...
function download(file_path) {
    window.open(file_path + '?download=file');
}

function do_action(act, t) {
    var heading;
    if (t == 'dir') {
        if (act == 'add')heading = "Введіть ім'я для папки";
        if (act == 'rename')heading = "Введіть ім'я для папки";
        if (act == 'delete') {
            form_submit(act, t);
            return;
        }
        ...
        if (act == 'download_all') {
            window.open('.' + get_path(1) + '?download=dir');
            return;
        }
        if (act == 'cut') {
            if (selected_dir_id == 1) {
                alert('Ви не можете вирізати кореневий каталог');
                return;
            }
            clipboard['empty'] = false;
            clipboard['type'] = 'dir';
            clipboard['path'] = get_path(selected_dir_id);
            clipboard['mode'] = 'cut';
            return;
        }
        if (act == 'copy') {
        ...
        if (act == 'paste') {
            if (clipboard['empty']) {
                alert('Буфер обміну порожній. Спочатку виріжіть або копіюйте потрібні файли.');
                return;
            }
            if (get_path(selected_dir_id).indexOf(clipboard['path']) == 0) {
                alert('Не можна копіювати/переміщати у вкладену папку');
                return;
            }
    ...
    if (t == 'file') {
        if (act == 'rename')heading = "Введіть нове ім'я для файлу";
        if (act == 'delete') {
            form_submit(act, t);
            return;
        }
        if (act == 'download') {
            window.open('.' + get_path(dir_id) + selected_file + '?download=file');
            return;
        }
        if (act == 'copy-public-link') {
            window.prompt("Public URL(Ctrl+C to copy to clipboard):", public_url_base + get_path(dir_id) + selected_file);
            return;
        }
        if (act == 'cut') {
            clipboard['empty'] = false;
            clipboard['type'] = 'file';
            clipboard['path'] = get_path(dir_id) + selected_file;
            clipboard['mode'] = 'cut';
            return;
        }
        if (act == 'copy') {
