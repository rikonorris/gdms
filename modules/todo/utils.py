...
def mark_done(request, done_items):
    ...
    for item in done_items:
        ...
        i.save()
        messages.success(request, "Задача \"{i}\" помічена вами як завершена".format(i=i.title))
...
def undo_completed_task(request, undone_items):
    ...
    for item in undone_items:
        ...
        i.completed = False
        i.save()
        messages.success(request, "Задачу \"{i}\" було повернуто на виконання".format(i=i.title))
...
def del_tasks(request, deleted_items):
    ...
    for item_id in deleted_items:
        ...
        messages.success(request, "Задачу \"{i}\" видалено".format(i=i.title))
        i.delete()
...
def send_notify_mail(request, new_task):
    ...
    try:
        ...
        messages.error(request, "Задача збережена але електронні листи не надіслано")
