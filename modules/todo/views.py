...
def list_lists(request):
    ...
    if request.user.groups.all().count() == 0:
        messages.error(request, "Ваш обліковий запит не додано до жодної групи. Попросіть адміністрацію аби додали вас до групи")
...
def del_list(request, list_id, list_slug):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, "{list_name} видалено.".format(list_name=list.name))
...
def view_list(request, list_id=0, list_slug=None, view_completed=False):
    ...
    if list_slug == "mine" or list_slug == "recent-add" or list_slug == "recent-complete":
        ...
        if list.group in request.user.groups.all() or request.user.is_staff or list_slug == "mine":
            ...
            messages.error(request, "У вас немає прав на перегляд даної категорії задач")
...
def view_task(request, task_id):
    ...
    if task.list.group in request.user.groups.all() or request.user.is_staff:
            ...
            if form.is_valid():
                ...
                if request.POST['comment-body']:
                    ...
                    try:
                        ...
                        messages.success(request, "Ваш коментар було надіслано усім учасникам задачі")
                    except:
                        messages.error(request, "Коментар не було надіслано! Зв'яжіться з адміністратором")
        ...
        messages.info(request, "У вас немає прав на перегляд чи зміну задачі")
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.error(request, "Задача збережена але електронний лист не відправлено. Повідомте про проблему адміністтратора.")
